...
(function() {
  ...
  // 普通单页模式翻页
    ...
    if (elements.viewer && pageSlider.slider) {
      ...
      let isLongPressMagnified = false;
      
      const startMagnify = (clientX, clientY) => {
        if (!pageSlider.isSinglePageMode() || !elements.currentImage) return;
        isLongPressMagnified = true;
        ...
        // 平滑放大 2.5倍
        elements.currentImage.style.transition = 'transform 0.2s ease-out';
        elements.currentImage.style.transformOrigin = `${xPercent}% ${yPercent}%`;
        elements.currentImage.style.transform = 'scale(2.5)';
      };

      const stopMagnify = () => {
        if (!isLongPressMagnified) return;
        isLongPressMagnified = false;
        ...
        if (elements.currentImage) {
          ...
          // 稍微延迟后恢复默认动画，防止缩放闪动
          setTimeout(() => {
            if (!isLongPressMagnified && elements.currentImage) {
              elements.currentImage.style.transition = 'opacity 0.25s ease, transform 0.25s ease';
            }
          }, 200);
        }
      };
      ...
      elements.viewer.addEventListener('touchstart', (e) => {
        ...
        if (handleDragStart(touch.clientX, touch.clientY, e)) {
          longPressTimer = setTimeout(() => {
            startMagnify(touch.clientX, touch.clientY);
          }, 350); // 350ms 触发长按
        }
      }, { passive: true });
      ...
      elements.viewer.addEventListener('touchmove', (e) => {
        ...
        // 移动距离过大，取消长按判定
        if (longPressTimer && pageSlider.isDragging) {
           const dx = touch.clientX - pageSlider.startX;
           const dy = touch.clientY - pageSlider.startY;
      ...
      elements.viewer.addEventListener('touchend', (e) => {
        ...
        if (isLongPressMagnified) {
          stopMagnify();
          pageSlider.justDragged = true; // 防止放大后松手误点屏幕触发了下一页
          setTimeout(() => { pageSlider.justDragged = false; }, 50);
          return;
        }
      ...
      document.addEventListener('mousemove', (e) => {
        ...
        // 移动距离过大，取消长按判定
        if (longPressTimer && pageSlider.isDragging) {
           const dx = e.clientX - pageSlider.startX;
           const dy = e.clientY - pageSlider.startY;
           if (Math.abs(dx) > 10 || Math.abs(dy) > 10) {
             clearTimeout(longPressTimer);
             longPressTimer = null;
           }
