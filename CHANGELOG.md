...
- **非虚拟纵向模式侧边留白实时更新**：侧边留白滑块调整后立即重新计算所有已加载图片的高度，不再需要重新进入模式才能生效。

## [2.4.8] - 2026-01-23
### Fixed
- **虚拟滚动模式间距/侧边距设置修复**：虚拟滚动模式（纵向连续模式超过 200 页时启用）的间距和侧边距设置现在能正确生效。之前设置滑块只更新了 CSS gap/padding，但虚拟滚动使用绝对定位布局，现在会正确重新计算 `virtualScroll.gap/sidePadding` 并更新所有已渲染卡片的位置。
- **单页模式拖拽翻页尺寸闪跳修复**：
  - 参考 JHenTai 的 `imageContainerSizes` 缓存策略，使用 `Image.decode()` 确保图片在重置滑轨位置前已完全解码
  - 翻页时调用 `resetImageZoom()` 重置缩放状态，与 `scheduleShowPage` 行为保持一致
  - 修复了从高分辨率图翻到低分辨率图时，图片先以原尺寸显示后才填充的问题

## [2.4.7] - 2026-01-23
### Fixed
- **单页模式拖拽翻页闪跳修复**：参考 JHenTai 的 `PhotoViewGallery` 预渲染模式，翻页动画完成时先将目标页图片同步复制到当前滑块，再重置滑轨位置。修复了从第1页翻到第2页时会"闪"一下第1页画面的问题。

### Added
- **连续模式鼠标拖拽滚动**：参考 JHenTai 同时支持手指和鼠标拖拽的设计，为所有连续模式（横向/纵向/虚拟滚动）添加鼠标拖拽滚动支持：
  - 按住鼠标左键拖拽即可滚动
  - 拖拽时显示 `grabbing` 光标
  - 拖拽结束后自动阻止误触发点击事件
  - 支持反向阅读模式的方向适配
...
## [2.4.6] - 2026-01-23
### Fixed
- **拖拽翻页菜单冲突修复**：单页模式拖拽翻页后不再错误触发顶栏/底栏菜单切换，添加 `justDragged` 标志阻止拖拽结束后的 click 事件
- **连续模式触摸滚动修复**：修复连续模式（横向/纵向）下触摸滚动被串珠滑轨阻挡的问题，现在 `enterContinuousHorizontalMode` 和 `enterContinuousVerticalMode` 会正确隐藏滑轨

## [2.4.5] - 2026-01-23
### Added
- **串珠式拖拽翻页**：单页模式新增类似 JHenTai/iOS PageView 的拖拽翻页效果，参考 JHenTai 的 `PhotoViewGallery` + `PageController.animateToPage` 实现：
  - 拖拽时可见前后页图片，像拉动串珠一样流畅
  - 三滑块轨道设计（prev/current/next），翻页时自然过渡
  - 15% 阈值或速度 > 0.5px/ms 触发翻页
  - 280ms 缓动动画，边界弹性回弹效果
  - 不与中心点击显示菜单冲突（微小拖动 < 15px 不触发翻页）
- **虚拟滚动调试日志**：`loadVirtualImage` 函数添加详细日志，方便排查图片加载问题

## [2.4.4] - 2026-01-23
### Fixed
- **跳转位置稳定性大修复**：参考 JHenTai 的 `scrollTo(index:)` 模式，新增 `pendingJumpTarget` 跟踪机制，跳转后 2 秒内持续以目标页为锚点调整滚动位置，修复以下问题：
  - 跳转到远处页面时被“挤”到错误位置（如跳到1192页却显示1204页）
  - 缩略图高亮和进度条显示与实际页面不同步
  - 轻微滚动后页码恢复但进度条未更新
- **布局重算稳定性**：`recalcVirtualOffsets`/`recalcVirtualOffsetsH` 在有跳转目标时优先以目标页为锚点，而不是视口中心元素
- **图片尺寸更新稳定性**：`applyVirtualAspect`/`applyVirtualAspectH` 在跳转期间跳过即时滚动补偿，由统一的布局重算处理

## [2.4.3] - 2026-01-23
### Fixed
- **模式切换间距一致性**：布局计算时先从 `state.imageCache` 提取所有已加载图片的真实尺寸到 `knownHeights/knownWidths`，确保已缓存图片和未缓存图片的布局计算一致。修复了从横向模式切换到纵向模式时，部分图片之间出现异常空白间距的问题。

## [2.4.2] - 2026-01-23
### Fixed
- **模式切换布局一致性**：参考 JHenTai 的 `clearImageContainerSized` 模式，布局计算不再使用 `ratioCache` 预计算，而是统一使用默认高度/宽度，图片加载后再动态调整。修复了从横向模式切换到纵向模式时，部分图片间距异常的问题。

## [2.4.1] - 2026-01-23
### Fixed
- **模式切换页码保持**：参考 JHenTai 的 `initialIndex = currentImageIndex` 模式，在模式切换前保存当前页码，切换后正确跳转到保存的页面，而不是重置到第 1 页。
- **虚拟滚动初始化保护**：在虚拟滚动初始化期间设置 `isJumping` 标志，防止初始渲染时的页码更新覆盖保存的页码。
- **模式切换状态清理**：模式切换时正确清理虚拟滚动状态对象，避免状态残留导致的问题。

## [2.4.0] - 2026-01-23
### Added
- **横向虚拟滚动**：横向连续模式也支持虚拟滚动，超过 200 页的画廊自动启用，大幅降低内存占用。

### Fixed
- **页码跳动问题修复**：参考 JHenTai 的 `filterAndSortItems` + `firstOrNull` 模式，改用"第一个可见元素"作为当前页，而不是"视口中心元素"，解决跳页后滚动时页码反复跳动的问题。
- **横向模式小图片填充**：修复横向虚拟滚动模式下小图片不能正确填充容器高度的问题，现在使用 `aspect-ratio` + `object-fit:contain` 与非虚拟模式保持一致。
- **跳转稳定性增强**：跳转页面时使用 `isJumping` 标志锁定页码更新，将目标元素定位到视口起始位置而不是居中，避免滚动事件干扰。

### Improved
- **JHenTai 模式对齐**：整体对齐 JHenTai 的滚动列表实现模式，包括可见元素过滤、排序、页码选取逻辑。

## [2.3.9] - 2026-01-23
### Added
- **虚拟滚动模式**：超过 200 页的画廊在纵向连续模式下自动启用虚拟滚动，只渲染可视区域 ± 8 页的元素，大幅降低内存占用。
- 参考 JHenTai 的 `ScrollablePositionedList` 实现，支持 2000+ 页的超大画廊流畅滚动。

### Improved
- **内存效率**：虚拟滚动模式下，1996 页画廊仅需渲染约 20 个 DOM 元素，而非 2000 个。
- **动态回收**：滚动时自动回收不可见元素，创建新进入视口的元素。
- **精准定位**：使用预计算的元素偏移量实现精准的页码跳转和滚动位置计算。

### Performance
- 虚拟滚动使用二分查找快速定位可见元素范围，滚动性能接近 O(log n)。
- 元素高度预计算基于图片宽高比缓存，避免滚动时的布局抖动。
...
### Improved
- **纵向连续模式优化**：对纵向连续阅读模式进行了重大调整，对条漫阅读体验有很大提升。
- **图片加载挤压修复**：解决了图片加载时布局挤压/抱动的问题，滚动更平稳。
